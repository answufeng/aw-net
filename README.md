...
- **依赖来源**：通过 [JitPack](https://jitpack.io/#answufeng/aw-net) 按 Git tag 版本引用；`implementation("com.github.answufeng:aw-net:TAG")` 中的 **TAG 需与你在 JitPack 选用的发布 tag 一致**。
...
- **CI**（[`.github/workflows/ci.yml`](.github/workflows/ci.yml)）：JDK 17 下对 `:aw-net:assembleRelease`、**ktlint**、`:demo:assembleRelease`（R8 冒烟）、**Android Lint** 做基础门禁。
...
- **本机 `java` 仍是 11？** 运行本项目需要让 **Gradle 使用 JDK 17**（`JAVA_HOME`、IDE 的 Gradle JDK，或见 [gradle.properties](gradle.properties) 顶部注释、CONTRIBUTING [环境](CONTRIBUTING.md#环境)）。
...
模块 `demo/` 为主界面卡片导航；工具栏菜单 **「演示清单」** 提供摘要，完整矩阵见 [demo/DEMO_MATRIX.md](demo/DEMO_MATRIX.md)。建议在 **JDK 17** 下执行 `./gradlew :demo:assembleRelease` 做冒烟。
...
`NetTracker` 单例在 `track` / `trackAsync` 中调用你提供的 `onEvent` 时已做防护：实现**抛错**时库内记 `Log.w`（`trackAsync` 另有 5s 超时），**不会**将异常再抛到 `executeRequest` 等主路径。仍建议在 `onEvent` 内自行 try/catch，避免无意义刷日志。
...
**Q: 如何自定义 OkHttp 配置？**
A: 优先通过 `NetworkConfig`（`NetworkConfigProvider` 初始读入的选项：超时、连接池、证书固定、HTTP 缓存、Cookie、重试等）与文档「运行时配置」表调整。自定义**应用**拦截器：在应用 Hilt 模块中 `@Provides` 带 `@AppInterceptor` 的 `Map<Int, Interceptor>`，与 `OptionalBindingsModule` 中的可选绑定对应。HTTP 的 `OkHttpClient` 由 `NetworkModule` 无限定符单例提供，若需与库默认完全不同的装配，请在工程中用 Hilt 的模块替换/排除或合并策略覆盖 `provideOkHttpClient`，**避免**再声明第二个无限定符的 `OkHttpClient` 造成重复绑定。WebSocket 可使用 `@WebSocketClient` 提供独立于 HTTP 的 `OkHttpClient`（见 `WebSocketModule` 与 `com.answufeng.net.websocket.annotation.WebSocketClient`）。
